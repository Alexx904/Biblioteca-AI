...
function Hero() {
  ...
  //Stato iniziale per contenere i dati in caricamento
  ...
  //Chiamata al backend al caricamento del componente
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setStats(data); // Aggiorna lo stato con i dati reali del DB
        }
      } catch (err) {
        console.error("Errore caricamento statistiche Hero:", err);
      }
    };
    ...
    <Box component="section" sx={{ py: 6, overflow: 'hidden' }}> {/*Box che contiene tutta la sezione Hero. Padding verticale per saccare dalla NavBar e overflow hidden nasconde gli elementi che escono dalla box*/}
      <Container maxWidth="lg"> {/*Limita la larghezza (maxWidth="lg"): Evita che il testo e le immagini si allunghino troppo sui monitor molto larghi, migliorando la leggibilità. Mette i contenuti al centro*/}
        ...
        <Stack direction='row' spacing={2} useFlexGap sx={{ justifyContent: 'center', flexWrap: 'wrap' }}> {/*flexWrap serve per mandare a capo gli elementi dello stack se non c'è spazio */}
          <HeroCard title='Volumi in catalogo' value={String(stats.libri)} />
          <HeroCard title='Postazioni studio' value={String(stats.postazioni)} />
          <HeroCard title='Orario di apertura' value={stats.orari} />
        </Stack>
      </Container>
    </Box>
  );
